...
Transaccion dataset_original[MAX_FILAS];
int total_lineas = 0;
char moda_global[50];
...
//Calcular la moda de las 3 categorías
void calcular_moda() {
    int cont_electronica = 0;
    int cont_hogar = 0;
    int cont_ropa = 0;
    for(int i = 0; i < total_lineas; i++) {

        if(strcmp(dataset[i].categoria, "ELECTRONICA") == 0)
            cont_electronica++;

        else if(strcmp(dataset[i].categoria, "HOGAR") == 0)
            cont_hogar++;

        else if(strcmp(dataset[i].categoria, "ROPA") == 0)
            cont_ropa++;
    }
     if(cont_electronica >= cont_hogar &&
       cont_electronica >= cont_ropa) {

        strcpy(moda_global, "ELECTRONICA");
    }

    else if(cont_hogar >= cont_ropa) {

        strcpy(moda_global, "HOGAR");
    }

    else {

        strcpy(moda_global, "ROPA");
    }
...
int main(int argc, char *argv[]) {
    ...
    dataset[total_lineas].id = atoi(token);
    ...
    if (token != NULL && strlen(token) > 0) {
        ...
        strcpy(dataset[total_lineas].categoria, "");
    }
